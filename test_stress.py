...
if __name__ == "__main__":
    ...
    _run("Import all packages", test_imports)
    _run("NumPy array operations", test_numpy)
    ...
    _run("Audio devices available", test_audio_devices)
    _run("Default input device", test_default_input)
    _run("Audio capture (direct)", test_audio_capture)
    _run("Audio capture (callback)", test_audio_capture_callback)
    ...
    _run("Model load", test_model_load)
    _run("Transcribe silence", test_transcribe_silence)
    _run("Transcribe tone (non-speech)", test_transcribe_tone)
    _run("Transcribe live speech (3s)", test_transcribe_speech)
    _run("Transcription speed benchmark", test_transcribe_speed)
    ...
    _run("Clipboard read/write", test_clipboard)
    _run("Keyboard hook (F9)", test_keyboard_hook)
    _run("System tray icon creation", test_tray_icon_create)
    _run("Sound files exist", test_sound_files)
    _run("TTS engine (pyttsx3)", test_tts_engine)
    ...
    _run("Full pipeline (record -> transcribe -> clipboard)", test_full_pipeline)
