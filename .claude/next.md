...
- [ ] **PR 1 — Finish live mic test of `feat/prompt-assist-v2` → merge → tag v4.4.0-beta1.** Re-run from source via `.\start.bat` with all 6 live-test fixes applied. Validate: Zira opener, beep cue, 2-slot Q&A (Task + Context only, NO Format), overlay with clean prompt + 4 buttons visible (Cancel/Add/Refine/Paste), voice-confirm ("say send"), Escape cancels. Whisper small model (already cached) should nail "Boca Tanning Club" cleanly.
