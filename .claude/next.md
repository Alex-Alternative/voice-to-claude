...
## Mac version (URGENT — coworker is on a Mac, session 55)

- [ ] **Build a Mac version of Koda.** Session 55 (2026-05-11) confirmed coworker tried to install KodaSetup-4.4.0-beta1.exe on a Mac → error `-10661` (`kLSIncorrectFormatErr` — Windows binary rejected by macOS LaunchServices). The whole "share with coworker" track is **blocked on Mac port** until either he switches to Windows or we ship a Mac build. Hard blockers: (a) PyInstaller does NOT cross-compile — must build on a Mac, (b) 8 modules import Win32-specific code (`win32`, `winreg`, `comtypes`, `pystray._win32`, `pyttsx3.drivers.sapi5`, `popen_spawn_win32`) — `voice.py`, `active_window.py`, `context_menu.py`, `formula_mode.py`, `prompt_conversation.py`, `settings_gui.py`, `test_features.py`, `build_exe.py`. (c) `koda.iss` is Inno Setup (Windows-only) — Mac equivalent is `.app` bundle wrapped in `.dmg` via `dmgbuild` or `create-dmg`. (d) Apple Developer account ($99/yr) needed for code signing + notarization, otherwise Gatekeeper warnings on the coworker's machine. (e) macOS permissions ceremony: Accessibility (global hotkeys + paste), Input Monitoring (key listening), Microphone. Realistic effort: several days of porting on a Mac dev box, multi-session project.

## Docs drift (session 50 → CLEARED session 55)

- [x] **`docs/user-guide.html` is stale (Apr 20).** DONE session 55 (2026-05-11). Full rewrite to v4.4.0-beta1 feature set: Hardware tier system, Prompt Assist v2 voice Q&A, Polish rename, Performance settings, Power Mode, voice-controlled app launch. Mirrored to .html with same Atlas Navy chrome. Now ALSO bundled in installer (`{app}\docs\user-guide.html`) + Start Menu shortcut "Koda User Guide" via PR #42 (`installer/koda.iss` [Files] + [Icons]).
...
- [ ] **Validate POWER tier on home PC with v4.4.0 installer** — auto-updater will trigger the toast on next home-PC Koda launch (current v4.3.1 → v4.4.0). Confirm sub-second to few-second transcription on RTX 4060. Now blocks nothing — PR #40 already merged, release already cut.
- [x] **Resolve Skill Forge uncommitted changes** — DONE session 55 (2026-05-11). Was `.codex/` + `AGENTS.md` artifacts blocking the currency check. Added both to `~/Projects/skillforge/.gitignore` (commit `d9c5c4d`, pushed direct to main with admin override). Currency check now reports clean.
- [x] **Run forge-deslop + forge-review on PRs #39 and #40** — DONE session 54 (2026-05-09). forge-deslop clean (0 findings); forge-review 1 NEEDS-FIX (model_downloader test coverage) + 1 NIT (canonical logger name) — both resolved. PR #40 also got 14 forge-clean cleanup commits + 8 new tests. 460/460 tests.
- [x] **Re-target PR #40 base to master** — DONE session 54. Re-targeted via `gh pr edit 40 --base master`, rebased onto current master (auto-skipped 4 already-applied phase-3 commits, resolved one `.claude/next.md` conflict by taking c08d73a's version), force-with-lease pushed. PR #40 mergeStateStatus: CLEAN, MERGEABLE.
...
- [ ] **Tighten `koda.iss` for friction-free upgrades** — installer currently errors / prompts if Koda is running during upgrade. Add to `[Setup]`: `CloseApplications=yes`, `RestartApplications=yes`, `AppMutex=KodaSingleInstance` (and add the matching mutex to Koda main loop). Result: re-running the installer over an existing install closes Koda, swaps the exe, relaunches — no prompts, no manual kill. **More urgent now that v4.4.0 is live and auto-update notifications are firing** — without this, users get a download URL but the install will prompt them to manually kill Koda first.
...
## Completed this session (work-PC session 55, 2026-05-11)

- [x] **Merged PR #40** (Phase 4 of 4 — hardware tier system: backward-compat config migration, startup tier re-detection, turbo model GH mirror, cuDNN bundle fix, updater semver filter, 14 forge-clean commits, +8 model_downloader tests)
- [x] **PR #41 — gitignore `docs/sessions/`** — forge-handover artifacts go machine-local from here on. 57 historical handovers left in place; new ones stop landing in the repo. Merged.
- [x] **PR #42 — User guide rewrite + bundle into installer** — full rewrite of `docs/user-guide.md` (Apr 20 v4.3.0 → May 11 v4.4.0-beta1 feature set: Hardware tier, Prompt Assist v2, Polish, Performance settings, Power Mode, voice app-launch, troubleshooting). Mirrored to `.html` with same Atlas Navy chrome. Bundled into installer at `{app}\docs\user-guide.html` + Start Menu shortcut "Koda User Guide" via `koda.iss` [Files] + [Icons]. Merged.
- [x] **PR #43 — Version bump 4.3.1 → 4.4.0** — `voice.py:107` was lagging two releases behind `koda.iss`. Bumped both in lockstep: `voice.py` VERSION → 4.4.0, `koda.iss` MyAppVersion 4.4.0-beta1 → 4.4.0, MyAppVersionNumeric → 4.4.0.0. Merged.
- [x] **Rebuilt installer** — `dist/KodaSetup-4.4.0.exe` (535 MB) — cuDNN bundled, user-guide bundled, v4.4.0 version. Confirmed in Inno Setup compile log.
- [x] **Tagged GitHub release v4.4.0** — https://github.com/Moonhawk80/koda/releases/tag/v4.4.0. Installer attached as release asset. Auto-updater on any running Koda (v4.3.1 or 4.4.0-beta1) will detect the new version on next launch and fire Windows tray toast: "Koda v4.4.0 is available (you have vX.Y.Z). Right-click tray icon → Check for updates."
- [x] **Cleared stale Skill Forge blocker** — added `.codex/` + `AGENTS.md` to `~/Projects/skillforge/.gitignore` (commit `d9c5c4d`). Currency check now reports clean. Unblocks pre-push gate for future sessions.
- [x] **Pre-push gate ran post-hoc on session diff** — forge-deslop clean (0 findings, 7 patterns), forge-review clean (0 findings, 6 layers, 460/460 tests, no claim/reality contradictions). Reports persisted to `.forge-deslop/run-*` and `.forge-review/run-*`.
- [x] **Confirmed coworker share blocked on Mac port** — KodaSetup-4.4.0-beta1.exe failed on coworker's Mac with error `-10661`. Entire coworker track now depends on Mac build.
